...
INPUT_SCHEMA = runtime_identity("subject-index-dimension-calculation-input-v2")
...
def validate_schema_document(document: dict[str, Any], schema_name: str, label: str, *, profile: str | None = None) -> None:
    errors = schema_errors(document, schema_name, profile=profile)
    if errors:
...
def finish_dimension(
    ...
    stable = lower_post == upper_post and lower_cap_id == upper_cap_id
    scored = not full_not_measured and not insufficient_component and stable
    contribution = central_post * Decimal(weight) / HUNDRED if scored else None
    lower_contribution = lower_post * Decimal(weight) / HUNDRED
    upper_contribution = upper_post * Decimal(weight) / HUNDRED
    ...
    return {
        ...
        "post_cap_percentage": decimal_text(central_post),
        "missing_data_bounds": {
            "lower": {
                "pre_cap_percentage": decimal_text(lower_base),
                "post_cap_percentage": decimal_text(lower_post),
...
def _resolved_selectivity(ledgers: dict[str, Any], audit_mode: str) -> dict[str, Any]:
    ...
    if percentage_native():
        result.update(substantive_selectivity_percentage=decimal_text(central_sub_post),
                      ...
                      density_points_out_of_5=decimal_text(density_rating * Decimal(5) / HUNDRED))
    return result
