...
Audit every packet assignment exactly once. Preserve the complete heading path and use only `supported`, `partially_supported`, `unsupported`, or `uninspectable`. `supported` means keep the locator unchanged; `partially_supported` and `unsupported` mean do not keep it as delivered. Judge treatment from independently useful information, not rhetorical form: comparative-list facts and attributed observations may be meaningful, while `attribution_only`, `citation_only`, and `incidental_example` apply only when the passage supplies no independently useful information about the complete heading path. Record page treatment and the required `complete_path_fit` field independently; fit must be `exact_fit`, `material_partial_fit`, `material_mismatch`, `severe_mismatch`, `no_fit`, or `uninspectable`. Add a concise evidence summary, any required fit rationale, evidence IDs, error codes, severity, and confidence. Preserve strict scope, relationship, chronology, compound-heading, attribution, and stance evaluation. Do not perform missing-access, global-structure, density, scoring, or reporting work.
