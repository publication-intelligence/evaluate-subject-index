...
- Canonical projection scorecards preserve `dimension_percentage` and `weighted_contribution` as exact decimal strings. They do not convert percentages to five-point ratings or coerce contributions to binary floating-point values. The density collection likewise exposes only its canonical `fit_percentage`, not a derived five-point rating.
- `build-report --replace-complete-bundle` may read the exact registered predecessor display aliases solely to validate and atomically replace that bundle. New output always has to satisfy the corrected current contract.
