...
6. `scripts/v10_cli.py page-chunks prepare-locator-chunks` validates that registered binding and prepares the complete locator-packet batch directly from local artifacts.

The current contract uses `candidate-layout-extraction-v1`, `candidate-index-v2`, optional `candidate-normalization-issues-v1`, `subject-index-item-inventory-v2`, and the current V10 evaluation state. The inventory remains canonical only because current scoring and structure commands consume it directly; it may be removed after those interfaces reconstruct it on demand. Superseded preparation formats are not accepted.
